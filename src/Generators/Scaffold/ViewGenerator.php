...
class ViewGenerator extends BaseGenerator
{
	...
	foreach ($this->commandData->fields as $field)
	{
	    ...
	    $tableBodyFields[] = fill_template_with_field_data(
		    $this->commandData->dynamicVars, $this->commandData->fieldNamesMapping, $cellFieldTemplate, $field
	    );
	}
	...
	foreach ($this->commandData->fields as $field)
	{
	    ...
	    $headerFields[]	 = $fieldTemplate	 = fill_template_with_field_data(
		    $this->commandData->dynamicVars, $this->commandData->fieldNamesMapping, $headerFieldTemplate, $field
	    );
	}
	...
	$searchable_fields = $this->commandData->getSearchableFields();

	$filter = array('$FILTER$' => "");

	for ($i = 0; $i < count($searchable_fields); $i++)
	{
	    $filterTemplate = get_template('scaffold.views.filter', $this->templateType);

	    $filter['$FILTER$'] .= fill_template($searchable_fields[$i], $filterTemplate);
	}

	$templateData = fill_template($filter, $templateData);
...
//            }
		...
		$fieldTemplate		 = fill_template_with_field_data(
			$this->commandData->dynamicVars, $this->commandData->fieldNamesMapping, $fieldTemplate, $field
		);
